...
def reinitialize_scheduler(optimizer, current_step, total_steps, new_warmup_steps, scheduler_type, min_lr_ratio, last_epoch=-1):
    # Calculate the remaining training steps
    remaining_steps = total_steps - current_step
    
    # Reinitialize the scheduler with the new warmup steps
...
def create_temporary_warmup_scheduler(optimizer, start_lr, end_lr, warmup_steps, last_epoch=-1):
    """
    Creates a LambdaLR scheduler for a temporary warmup phase in training. During the warmup phase,
    the learning rate linearly increases from `start_lr` to `end_lr` over a defined number of steps (`warmup_steps`).
    After the warmup phase, the learning rate is maintained at `end_lr`.
    
    Args:
        optimizer (Optimizer): The optimizer linked with the scheduler.
        start_lr (float): The initial learning rate at the beginning of the warmup phase.
        end_lr (float): The target learning rate to achieve by the end of the warmup phase.
        warmup_steps (int): The number of steps over which the warmup should occur.
        last_epoch (int): The index of the last epoch. This is used when resuming training
                          and affects the initial learning rate calculation. Default is -1,
                          which starts the scheduler from the beginning.
    
    Returns:
        LambdaLR: A LambdaLR scheduler configured for a temporary warmup phase.
    """
    def lr_lambda(step):
        # Default learning rate from the optimizer, used for normalization
        default_lr = optimizer.defaults['lr']
        
        if step < warmup_steps:
            # Calculate warmup progress as a fraction of the total warmup steps
            warmup_progress = step / float(max(1, warmup_steps))
            
            # Linearly interpolate learning rate based on warmup progress
            lr = (end_lr - start_lr) * warmup_progress + start_lr
            
            # Normalize interpolated learning rate by default optimizer learning rate
            return lr / default_lr
        else:
            # Maintain learning rate at `end_lr` after warmup phase
            return end_lr / default_lr
...
# Function to get the current learning rate from the scheduler
def get_current_lr(scheduler):
    return scheduler.get_last_lr()[0]  # Assuming the scheduler has a get_last_lr method
