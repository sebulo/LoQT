...
def evaluate_model(model, preprocess_batched, pad_idx, global_rank, world_size, device, batch_size, dataset=None, tokenizer=None):
    ...
    _time = time.time()
    if dataset is None:
        val_data = datasets.load_dataset("allenai/c4", "en", split="validation", data_files={'validation': 'en/c4-validation.00000-of-00008.json.gz'}, streaming=False, trust_remote_code=True)
    else:
        val_data = dataset
